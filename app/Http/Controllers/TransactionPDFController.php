...
class TransactionPDFController extends Controller
{
    function index()
    {
     ...
     return view('client.transaction_pdf')->with('customer_data', $customer_data);
    }

    function get_customer_data()
    {
     $customer_data = DB::table('transactions')
         ->limit(10)
         ->get();
    ...
    <th style="border: 1px solid; padding:12px;" width="20%">Country</th>
     ...
     foreach($customer_data as $customer)
     {
      ...
      <tr>
       ...
       <td style="border: 1px solid; padding:12px;">'.$customer->credit.'</td>
       <td style="border: 1px solid; padding:12px;">'.$customer->credit_type.'</td>
       <td style="border: 1px solid; padding:12px;">'.$customer->debit.'</td>
       <td style="border: 1px solid; padding:12px;">'.$customer->debit_type.'</td>
